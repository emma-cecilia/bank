<?php 
	class employe_creerCompte
	{
		...
		{			
			...
			public function addAjout_employe()
		{
			$bdd = $this->connexionBdd();
					
			//chemin pour id postlant
			$request = $bdd->prepare("SELECT `id_admin` FROM `administrateurs` ORDER BY id_admin DESC") ;
			...
			$creer_compte_id_creercompte = $result[0]['id_admin'];
/*--------------------------------------------------------------------------*/
			$request = $bdd->prepare("INSERT INTO employes
										         (id_emp,numero_matricule_emp,nom_emp,prenom_emp,
												 date_naissance_emp,email_emp,motdePass_emp,id_admin)
                                                   VALUES (NULL,?,?,?,?,?,?,?)");
			
			$reponse = $request->execute(array(	
												$_POST['numero_matricule_emp'],
												$_POST['nom_emp'],
												$_POST['prenom_emp'],
												$_POST['date_naissance_emp'],
												$_POST['email_emp'],
												$_POST['motdePass_emp'],
												
												//r�cup�ration de l'id principal
												$creer_compte_id_creercompte
											));
		...
		/*METHODE QUI AJOUTE UN COMPTE EMPLOYE*/
		// public function ajoutEmploye(){
			
			// $bdd = $this->connexionBdd();
			
			// requ�te pour �viter la redendance des noms
			// $request = $bdd->prepare("SELECT * FROM employes WHERE  = :nom_emp");
			// $result = $request->execute(array(':nom_emp'=>$this->nom_emp));
			...
			// $result=$request->fetchAll();
			// Affectation de l'creer_compte_id_creercompte dans l'id de etrang�re  
			// $id_admine = $result[0]['id_admin'];
// /*--------------------------------------------------------------------------*/
				// $request = $bdd->prepare("INSERT INTO employes
										        // (id_emp,numero_matricule_emp,nom_emp,prenom_emp,
												// date_naissance_emp,email_emp,motdePass_emp,id_admin) 
										 
										// VALUES(:id_emp,:numero_matricule_emp,:nom_emp,:prenom_emp,
											 // :date_naissance_emp,  :email_emp,:motdePass_emp,:id_admin)");
				
				// $reponse = $request->execute(array(
													// 'id_emp'	=>$this->id_emp,
													// 'numero_matricule_emp' =>$this->numero_matricule_emp,													
													// 'nom_emp'	=>$this->nom_emp,
													// 'prenom_emp'=>$this->prenom_emp,
													// 'date_naissance_emp'=>$this->date_naissance_emp,
